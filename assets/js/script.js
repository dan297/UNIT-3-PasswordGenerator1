...
function writePassword() {
  ...
  if (length < 8 || length > 128) {
    ...
    var upperCasetrue = confirm("Do you want uppercase lettering");
    var numberstrue = confirm("Do you want numbers?");
    var specialCharactortrue = confirm("Do you want special characters?");

    if (lowercasetrue) {
      randomfigure.push(lowercase);
    }
    if (upperCasetrue) {
      randomfigure.push(upperCase);
    }
    if (numberstrue) {
      randomfigure.push(numbers);
    }
    if (specialCharactortrue) {
      randomfigure.push(specialCharactor);
    }
